...
def Clustering(point_cloud, eps, min_points):
    ...
    if len(labels) > 0:
        ...
        for label in range(min_label + 1, max_label + 1):
            # np.where 函数会返回一个满足条件的元素的索引。
            # 在这里，它会返回 labels 数组中所有等于当前 label 的元素的索引。
            # 例如，如果 labels 是 [0, 0, 1, -1, 2, 2, 2]，并且当前 label 是 2，
            # 那么 label_index 将包含 2 的索引 [4, 5, 6]。
...
def call_back(pcd, img, index):
    try:  # ----------------------------------------原始数据获取---------------------------------------------------
        ...
        if len(objects) > 0:
            ...
            # ----------------------特征点提取-----------------------------------
            tail_points = np.asarray(tail_point_cloud.points)
            tail_points_list = tail_points.tolist()
            # print('船尾面的点', np.asarray(tail_point_cloud.points))
            try:
                with open('data.json', 'w') as f:
                    print(tail_points)
                    print('将数据记录下来')
                    json.dump(tail_points_list, f)
            except Exception as e:
                print(f'发生错误: {e}')
            lt_point = test.findlefttoppoint()
            lt_point = np.array(lt_point[0])
            print('不用聚类的角点坐标', lt_point)
            corner_pcd = Clustering(tail_point_cloud, eps=0.5, min_points=20)
            print(np.asarray(corner_pcd.points).shape)
            ...
            # 看凸包
            hull, idx = tail_ROI_pcd.compute_convex_hull()
            hull_cloud = tail_ROI_pcd.select_by_index(idx)
            hull_ls = o3d.geometry.LineSet.create_from_triangle_mesh(hull)
            hull_ls.paint_uniform_color((1, 0, 0))
            print('画图前')
            o3d.visualization.draw_geometries([tail_ROI_pcd,hull_ls])
            print('画图后')
            ...
            # XandZ = corner_points
            # # print(XandZ)
            # # 使用 argsort 根据第三列（z 坐标）进行排序
            # sorted_indices = np.argsort(XandZ[:, 2])
            #
            # # 使用排序后的索引来获取排序后的数组
            # sorted_x_and_z = XandZ[sorted_indices]
            #
            # # print(sorted_x_and_z)
            #
            # # 获取第二列的最大值
            # # max_value = np.amax(sorted_x_and_z[:, 1])
            # # print(max_value)
            #
            # # 获取第二列的最小值
            # min_value = np.amin(sorted_x_and_z[:, 2])
            # # print(min_value)
            #
            # # 范围
            # step = 0.5
            # # 数组存不同范围间点数
            # start_z = min_value + step
            # # 算点数
            # point_num = 0
            # #求最大值点的区间中线,和最大点数
            # max_point = 0
            # max_middle_line = start_z + step
            #
            # # 遍历所有点
            # for point in sorted_x_and_z:
            #     x, y, z = point
            #     if (z > start_z + step or z < start_z - step):
            #         if (point_num > max_point):
            #             max_middle_line = start_z
            #             max_point = point_num
            #         start_z += step
            #         point_num = 0
            #     else:
            #         point_num += 1
            #
            # # filtered_points = points[np.logical_and(sorted_x_and_z[:, 1] >= max_middle_line-step, sorted_x_and_z[:, 1] <= max_middle_line+step)]
            # # print('测试',filtered_points)
            # # 使用np.where找到满足条件的索引
            # indices = np.where(
            #     (sorted_x_and_z[:, 2] >= max_middle_line - step) & (sorted_x_and_z[:, 2] <= max_middle_line + step))
            # filtered_points = sorted_x_and_z[indices]
            # # print(filtered_points)
            # ng_x = -filtered_points[:, 0]
            # weight_sum = ng_x + filtered_points[:, 2]
            # max_index = np.argmax(weight_sum)
            # print(filtered_points[max_index])
            # # 根据index去找点
            # left_top_point2 = filtered_points[max_index]
            # left_top_point2 = [left_top_point2]
            # print('测试改进后的角点', left_top_point2)
            # 使用索引访问满足条件的点
            #-------------------------------------------------------
            ...
            left_top_point = [left_top_point, lt_point]

            left_top_point2 = test.findlefttoppoint()

            print("left_top_point:", left_top_point)
            print("left_top_point2:", left_top_point2)
