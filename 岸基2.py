...
# 创建文件处理器，使用追加模式
file_handler = logging.FileHandler('原算法2024-10-17.log', mode='a')
...
def call_back(pcd, img, index):
    try:
        ...
        print(time.time())
        # 6. 再次拟合船尾平面
        plane_model, inliers = tail_point_cloud.segment_plane(distance_threshold=0.005, ransac_n=3, num_iterations=100000)
        print(time.time())
        [a, b, c, d] = plane_model
        print("船尾面平面方程参数:")
        print(f"Plane equation: {a:.6f}x + {b:.6f}y + {c:.6f}z + {d:.6f} = 0")
        ...
        try:
            ...
            print(f'发生错误: {e}')
        ltPoint_hull = test.findlefttoppoint()
        ltPoint_hull = ltPoint_hull[0].tolist()
        print(f'凸包坐标{ltPoint_hull}')
...
if __name__ == '__main__':
    # 指定存储点云文件（.pcd）和图像文件（如.jpg或.png）的文件夹路径。
    # pcd_folder = r"E:\研究生期间资料\岸基项目\图片与点云\2024-10-17\pcd\20241017"
    pcd_folder = r"C:\Users\Yang Yuhao\Desktop\找点云工作\2024-10-07-08新点云文件\imgs\new image"
    image_folder = r"C:\Users\Yang Yuhao\Desktop\找点云工作\2024-10-07-08新点云文件\imgs\20241017"
    ...
    # 通过zip将点云文件和图像文件的名称配对，准备逐对处理。
    index = 1
    for pcd_file_path, image_file_path in zip(sorted(pcd_file_paths), sorted(image_file_paths)):
        print("-----------------------------------------------------------------")
        print("开始：", time.time())
        print("序号：", index)
