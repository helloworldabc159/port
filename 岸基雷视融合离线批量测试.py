...
def call_back(pcd, img, index):
    ...
    if len(objects) > 0:
        ...
        # 使用MSAC算法拟合平面
        print(time.time())
        params, inliers = MSAC_GPU(part_tail_points)
        print(time.time())

        # 打印平面方程参数
        print("YOLO识别面方程参数:")
        print(f"a: {params['a']}, b: {params['b']}, c: {params['c']}, d: {params['d']}")
        # a = cp.asnumpy(params['a'])
        # b = cp.asnumpy(params['b'])
        # c = cp.asnumpy(params['c'])
        # d = cp.asnumpy(params['d'])
        ...
        # 使用MSAC算法拟合平面并去除外点
        print(time.time())
        tail_params, tail_inliers = MSAC_GPU(whole_tail_points)
        print(time.time())
        # 将内点转换回点云
