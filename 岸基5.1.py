...
import os
import time
import Save_Visualization_As_Image
...
# 创建文件处理器，使用追加模式
file_handler = logging.FileHandler('测.log', mode='a', encoding='utf-8')
...
def call_back(pcd, img, index,filename):
    try:
        # ----------------------------------------原始数据获取---------------------------------------------------
        # 记录原始点云数据
        ...
        # logger.info(f"识别结果：{detections}")
        print(f"Plane equation: {a:.6f}x + {b:.6f}y + {c:.6f}z + {d:.6f} = 0")
        print('船尾面点云数量', np.asarray(tail_point_cloud.points).shape)

        # ----------------------特征点提取-----------------------------------
        # 读取点云
        # 聚类
        # 从点云中提取特征点（如角点），通过聚类算法将点云数据分组，以便后续操作。
        corner_pcd = Clustering(tail_point_cloud, eps=0.5, min_points=5)
        ...
        # logger.info(f"新角点点云坐标：{left_top_point2}")
        # --------------------------找到最左上角的角点-----------------------------
        # 方案一
        # 最小x坐标
...
if __name__ == '__main__':
    ...
    for pcd_file_path, image_file_path in zip(pcd_file_paths, image_file_paths):
        ...
        name_without_extension = pcd_file_path.split('.')[0]
        ...
        # 是您之前的代码片段中实现的主要处理逻辑。
        call_back(pcd, image, index,name_without_extension)
        index = index + 1
