...
def call_back(pcd, img, index, pcd_file_path, image_file_path, data):
    try:  # ----------------------------------------原始数据获取---------------------------------------------------
        # 记录原始点云数据
...
if __name__ == '__main__':
    ...
    pcd_folder = r"C:\Users\Yang Yuhao\Desktop\找点云工作\2024-10-07-08新点云文件\lidar\20241017"
    ...
    for pcd_file_path, image_file_path in zip(pcd_file_paths, image_file_paths):
        ...
        data = pd.DataFrame(
            columns=['点云文件名', '图像文件名', '点云角点x', '点云角点y', '点云角点z', 'nez坐标n', 'nez坐标e',
                     'nez坐标z'])
        call_back(pcd, image, index, pcd_file_path, image_file_path, data)
        index = index + 1
