...
# 创建文件处理器，使用追加模式
file_handler = logging.FileHandler('log文件夹/测试俯仰角与姿态角.log', mode='a',encoding='utf-8')
...
# 从 .mat 文件中加载数据
mat_4x4 = scipy.io.loadmat(r'chuanwei4x4.mat')
mat_3x3= scipy.io.loadmat(r'chuanwei3x3.mat')
# YOLO检测相关内容
# 初始化目录
...
def call_back(pcd, img, index):
    try:
        ...
        [
            # points[:, 0] >= 48, points[:, 0] <= 52,  # 水平方向过滤
        ...
        # o3d.visualization.draw_geometries([colored_cloud], window_name="yolo面")
...
#  -------------------------------区域生长分割--------------------------------------------------
        ...
        #
        # # 可视化处理后的点云
        # o3d.visualization.draw_geometries([inlier_cloud], window_name="区域生长分割")

# #-------------------------------语义分割-----------------------------------------------------
#         # image01 = cv2.imread(r"C:\Users\huangzhifei\Desktop\pcd\picture\2023-04-02_13_47_13.jpg")
#         # # # model = YOLO(r"F:\yolov5-master02\yolov5-master02\runs\train-seg\exp6\weights\best.pt")
#         # model = YOLO(r"F:\ultralytics-8.2.0\ultralytics\models\yolo\segment\runs\segment\train\weights\best.pt")
#         model = YOLO(r"D:\python\python1\yolov5-master\best.pt")
...
#         mask_bool = np.zeros((img.shape[0], img.shape[1]), dtype=np.uint8)
#
#         # 将边界点转换为填充掩膜
#         for mask in masksxy:
#             # 将点转换为整数并填充
...
#
#             # 检查是否在图像范围内且在掩膜内
#             if 0 <= u < img.shape[1] and 0 <= v < img.shape[0] and mask_bool[v, u]:
#                 color = image[v, u]  # 提取图像中的颜色信息
#                 colored_points.append(
#                     [point[0], point[1], point[2], color[2] / 255.0, color[1] / 255.0, color[0] / 255.0]  # RGB归一化
#                 )
...
#         print(len(colored_points))
#         colored_cloud = o3d.geometry.PointCloud()
#         if colored_points:  # 确保有有效的点
#             colored_cloud.points = o3d.utility.Vector3dVector(np.array(colored_points)[:, :3])  # 点坐标
#             colored_cloud.colors = o3d.utility.Vector3dVector(np.array(colored_points)[:, 3:])  # RGB颜色
#             print("有效的点数量:", len(colored_points))
#
#             # 可视化
...
#
#         # if len(colored_points) == 0:
#         #     logger.warning("掩膜内无点云")
#         # return

# # ------------------------------------------------------语义分割-----------------------------------
        # ---------------------------------------拟合-----------------------------------------------------
        print(time.time())
        plane_model, inliers = point_cloud.segment_plane(distance_threshold=0.06, ransac_n=3, num_iterations=20000)
        print(time.time())
        [a, b, c, d] = plane_model
        print("YOLO识别面方程参数:")
        ...
        after_filter = np.logical_and.reduce([
            a * raw_points[:, 0] + b * raw_points[:, 1] + c * raw_points[:, 2] + d >= -0.2,
            a * raw_points[:, 0] + b * raw_points[:, 1] + c * raw_points[:, 2] + d <= 0.2
        ])
        ...
        #----------------------------------------使用距离公式筛选出来的点-------------------------------------
        tail_point_cloud1 = o3d.geometry.PointCloud()

        # 计算每个点到平面的距离
        distances = np.abs(a * raw_points[:, 0] + b * raw_points[:, 1] + c * raw_points[:, 2] + d) / np.sqrt(
            a ** 2 + b ** 2 + c ** 2)

        # 设置距离阈值
        distance_threshold = 1  # 根据需要调整

        # 过滤条件
        after_filter = distances <= distance_threshold
        tail_point_cloud1.points = o3d.utility.Vector3dVector(raw_points[after_filter])
        tail_point_cloud1.paint_uniform_color([1, 0, 0])
        # o3d.visualization.draw_geometries([tail_point_cloud1], window_name=f"船尾平面点云使用距离公式_{index}")

        #----------------------------------------把yolo识别面的平面展示出来-----------------------------------
        inlier_points = raw_points[inliers]
        ...
        inlier_cloud.paint_uniform_color([0, 0, 1])  # 蓝色

        # ----------------------------------------展示无限延伸的平面---------------------------------------------------
        # def create_infinite_plane(a, b, c, d, size=100.0):
        #     # 生成一个大的矩形网格
        #     x_range = np.linspace(-size, size, 100)
        #     y_range = np.linspace(-size, size, 100)
        #     X, Y = np.meshgrid(x_range, y_range)
        #     Z = (-d - a * X - b * Y) / c  # 计算对应的 Z 值
        #     return X, Y, Z
        #
        # # 获取矩形网格的点
        # X, Y, Z = create_infinite_plane(a, b, c, d, size=100.0)
        #
        # # 创建平面几何体
        ...
        #         triangles.append([i * len(Y) + j, (i + 1) * len(Y) + j, i * len(Y) + (j + 1)])
        #         triangles.append([(i + 1) * len(Y) + j, (i + 1) * len(Y) + (j + 1), i * len(Y) + (j + 1)])
        # plane_mesh.triangles = o3d.utility.Vector3iVector(triangles)
        # plane_mesh.paint_uniform_color([0, 1, 0])  # 绿色

        # 可视化
        # o3d.visualization.draw_geometries([point_cloud, plane_mesh],
        #                                   window_name="Infinite Plane Visualization")

        # ---------------------------------------加上最小二乘法----------------------------------------------
        # 3. 使用最小二乘法进行平面拟合
        # raw_points = np.asarray(colored_cloud.points)
        # # 提取 x, y, z 坐标
        # X = raw_points[:, 0]
        # Y = raw_points[:, 1]
        # Z = raw_points[:, 2]
        ...
        #
        # # C 包含了平面方程的系数
        # a, b, c = C[0], C[1], -1  # c = -1，因为我们希望方程形式为 ax + by + cz + d = 0
        #
        # # 输出平面方程
        # d = C[2]
        # print(f"平面方程: {a:.2f}x + {b:.2f}y + {d:.2f} = z")
        # tail_point_cloud1 = o3d.geometry.PointCloud()
        # after_filter = np.logical_and.reduce([
        ...
        # tail_point_cloud1.points = o3d.utility.Vector3dVector(raw_points[after_filter])
        # tail_point_cloud1.paint_uniform_color([1, 0, 0])
        # o3d.visualization.draw_geometries([tail_point_cloud1], window_name=f"最小二乘法的船尾点云_{index}")
        ...
        # print(time.time())
        # # 6. 再次拟合船尾平面
        # plane_model, inliers = tail_point_cloud.segment_plane(distance_threshold=0.3, ransac_n=3, num_iterations=100000)
        # print(time.time())
        # [a, b, c, d] = plane_model
        # print("船尾面平面方程参数:")
        # # logger.info(f"识别结果：{detections}")
        ...
        #     print(f'发生错误: {e}')
        #
        # left_top_point2 = test3.findlefttoppoint()
        #
        # corner = o3d.geometry.PointCloud()
        # corner.points = o3d.utility.Vector3dVector(left_top_point2)
        ...
        # centroid_sphere.translate(left_top_point2[0])
        # centroid_sphere.paint_uniform_color([1, 0, 0])  # 质心上色为红色

        # o3d.visualization.draw_geometries([centroid_sphere, point_cloud], window_name=f"凸角点提取_{index}")
        ...
        # 方案二
        # 取y坐标
        # neg_y = corner_points[:, 1]
        # # 计算加权和(加权因子可调)
        # weighted_sum = 0.6 * neg_y + 0.4 * corner_points[:, 2]
        # # 找到和最大的点的索引
        ...
        # print("角点点云坐标：", np.asarray(corner.points))
        # logger.info(f"角点点云坐标：{np.asarray(corner.points)}")
        # test3.append_data_to_excel('newdata.xlsx',np.asarray(corner.points)[0])
        # #
        # centroid_sphere = o3d.geometry.TriangleMesh.create_sphere(radius=0.1)
        # centroid_sphere.translate(left_top_point[0])
        # centroid_sphere.paint_uniform_color([1, 0, 0])  # 质心上色为红色

        # o3d.visualization.draw_geometries([centroid_sphere, point_cloud], window_name=f"旧角点提取_{index}")
# ---------------------------------------------新旧角点算法分界线----------------------------------------------
#         def find_left_top_point(corner_points):
#             # 方案一：找到最左侧的点
#             # 使用np.argmin函数找到corner_points数组中第一列（即y坐标）的最小值的索引
#             max_y_index = np.argmax(corner_points[:, 1])
#             # 创建一个子数组left_point_candidates，它包含所有x坐标等于corner_points中最小y坐标值的点。这些点都是最左侧的点。
#             left_point_candidates = corner_points[corner_points[:, 1] == corner_points[max_y_index, 1]]
#
#             # 方案二：在左侧的点中找到最上方的点
#             # 计算left_point_candidates中每个点的x坐标的相反数，存储在neg_x中。
#             neg_y = left_point_candidates[:, 1]
#             # 每个点计算加权和，其中x坐标的相反数权重为0.6，z坐标的权重为0.4。这个计算是为了在y坐标相同的情况下，找到z坐标更高的点。
#             weighted_sum = 0.6 * neg_y + 0.4 * left_point_candidates[:, 2]
...
#
#         # 可视化
#         o3d.visualization.draw_geometries([centroid_sphere, point_cloud], window_name="c角点提取")
        # -----------------------------------坐标变换---------------------------------------
        # 从 .mat 文件中加载数据
...
if __name__ == '__main__':
    ...
    # pcd_folder = r"E:\找角点\20241117数据\lidar-part1-ship1"
    # image_folder= r"E:\找角点\20241117数据\cam-20241117\cam-20241117"
    # 船2
    pcd_folder = r"C:\Users\Yang Yuhao\Desktop\a"
    image_folder = r"C:\Users\Yang Yuhao\Desktop\连续10个img"
    while True:
        pcd = o3d.io.read_point_cloud(r"C:\Users\Yang Yuhao\Desktop\连续10个lidar\2024_11_20_09_30_12.pcd")
        image = cv2.imread(r'C:\Users\Yang Yuhao\Desktop\a\2024_11_20_09_30_12.jpg')
        call_back(pcd, image, 1)
    # pcd_folder = r"E:\post gratuation\anji\zBigJumpPcd"
    # image_folder= r"E:\post gratuation\anji\zBigJumpImgs"
    # pcd_folder = r"E:\post gratuation\anji\imgandpcd\2024-10-17\pcd\20241017"
    # image_folder = r"E:\post gratuation\anji\imgandpcd\2024-10-17\img\20241017"
