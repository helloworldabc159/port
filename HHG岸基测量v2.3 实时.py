...
# 从 .mat 文件中加载数据
mat_4x4 = scipy.io.loadmat(r'C:\Users\wxw\Desktop\yolov5-master\transfMtx_HHG.mat')
mat_3x3 = scipy.io.loadmat(r'C:\Users\wxw\Desktop\yolov5-master\RotfMtx.mat')
...
def run_shell_script(script_path):
    try:
        ...
        result = subprocess.run(['bash', script_path], check=True, text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def call_back(ros_pcd, ros_img):
    ...
    try:
        # 获取新点云
        part_tail_points = np.asarray(colored_cloud.points).T  # 转换为MSAC所需的格式
        # 使用MSAC算法拟合平面
        params, inliers = MSAC(part_tail_points)
        # 打印平面方程参数
        print("YOLO识别面方程参数:")
    ...
    # ----------------------按YOLO识别结果过滤原始点云-----------------------------------------------------
    try:
        whole_tail_points = np.asarray(tail_point_cloud.points).T
        # 使用MSAC算法拟合平面并去除外点
        tail_params, tail_inliers = MSAC(whole_tail_points)
        # 打印平面方程参数
        print("船尾面平面方程参数:")
        print(f"a: {tail_params['a']}, b: {tail_params['b']}, c: {tail_params['c']}, d: {tail_params['d']}")
        logger.info("船尾面平面方程参数:")
        logger.info(f"a: {tail_params['a']}, b: {tail_params['b']}, c: {tail_params['c']}, d: {tail_params['d']}")
    except Exception as e:
        logger.error(e)
        raise e
    # ----------------------特征点提取-----------------------------------
    ...
    if corner_pcd == "error":
        print("聚类出错，船尾面点云过于稀疏")
        logger.warning("聚类出错，船尾面点云过于稀疏")
        return
...
if __name__ == '__main__':
    ...
    ros_img = message_filters.Subscriber("/topic", Image, queue_size=1)
