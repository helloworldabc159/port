...
mat_3x3 = scipy.io.loadmat(r'/home/nvidia/yolov5-master/chuanwei33_1119.mat')
# --------------------------------------------------------
...
# 函数功能: 读取原始点云数据 强度这一列单独返回
def read_pcd_with_intensity_ascii(pcd_file, readline=11):
    data = []
    with open(pcd_file, 'r') as f:
        lines = f.readlines()[readline:]  # 从11行开始读取所有行
        # 以下循环是处理所有数据
        for line in lines:
            # print(line)
...
def call_back(ros_pcd, ros_img):
    try:
        ...
        for detection in detections:
            ...
            obj['real_coordinate'] = real_coordinate  # 将计算得到的真实坐标存储在物体字典的 real_coordinate 键中
            i = i + 1
            objects.append(obj)
        # ----------------------------------------数据融合-----------------------------------------------------
        if len(objects) > 0:
            ...
            print("YOLO识别面方程参数: ")
            print(f"第一次拟合得到YOLO识别面方程及参数: {a:.6f}x + {b:.6f}y + {c:.6f}z + {d:.6f} = 0")
            ...
            # 条件过滤: 以提取位于船尾平面附近的点
            after_filter = np.logical_and.reduce([
                a * raw_points[:, 0] + b * raw_points[:, 1] + c * raw_points[:, 2] + d >= -1,
                a * raw_points[:, 0] + b * raw_points[:, 1] + c * raw_points[:, 2] + d <= 1
            ])
            ...
            # -------------------------再次拟合-----------------------------------------------------
            # print("第二次平面拟合开始时间: ", time.time())
            # 6. 再次拟合船尾平面
            plane_model, inliers = tail_point_cloud.segment_plane(distance_threshold=0.05, ransac_n=3,
                                                                  num_iterations=100000)
            # print("第二次平面拟合结束时间: ", time.time())
            [a, b, c, d] = plane_model
            print(f"第二次拟合得到船尾面方程及参数: {a:.6f}x + {b:.6f}y + {c:.6f}z + {d:.6f} = 0")
            ...
            centroid_ts = np.dot(transfMtx,np.append(centroid_id,1))

            centroid_ts = [centroid_ts[0],-centroid_ts[1],-centroid_ts[2] + 0.32]

            print(f"zhi_xin_NEZ_zuobiao : {centroid_ts}")
            logger.info(f"zhi_xin_NEZ_zuobiao : {centroid_ts}")
            # -----------------------------------位姿计算---------------------------------------
            # 测试函数
