...
# 创建文件处理器，使用追加模式
file_handler = logging.FileHandler('旧算法ran0.25.log', mode='a', encoding='utf-8')
...
def call_back(pcd, img, index):
    try:
        ...
        after_filter = np.logical_and.reduce(
            [
                points[:, 0] >= 46, points[:, 0] <= 100,  # 水平方向过滤
        ...
        # ---------------------------------------拟合-----------------------------------------------------
        print(time.time())
        plane_model, inliers = colored_cloud.segment_plane(distance_threshold=0.25, ransac_n=3, num_iterations=100000)
        print(time.time())
        [a, b, c, d] = plane_model
        print("YOLO识别面方程参数:")
        ...
        # -----------------------------------测试切割方法------------------------------------------------
        print([a,b,c,d])
        cut_result = cut_railing1.cut_rail(a, b, c, d, np.asarray(tail_point_cloud.points),
                                           np.asarray(point_cloud.points),)
        cut_pcd = o3d.geometry.PointCloud()
        cut_pcd.points = o3d.utility.Vector3dVector(cut_result)
        # 可视化
        o3d.visualization.draw_geometries([cut_pcd], window_name="切割后的效果")

        # ----------------------按YOLO识别结果过滤原始点云-----------------------------------------------------

        print(time.time())
        # 6. 再次拟合船尾平面
        plane_model, inliers = tail_point_cloud.segment_plane(distance_threshold=0.25, ransac_n=3,
        ...
        #     print(f'发生错误: {e}')
        #
        # left_top_point2 = test.findlefttoppoint()
        #
...
if __name__ == '__main__':
    ...
    pcd_folder = r"C:\Users\Yang Yuhao\Desktop\a\pcds"
    image_folder = r"C:\Users\Yang Yuhao\Desktop\a\imgs"
    # pcd_folder = r"C:\Users\lenovo\Desktop\pcd"
    # image_folder= r"C:\Users\lenovo\Desktop\img"

    # pcd_folder = r"E:\post gratuation\anji\zBigJumpPcd"
    # image_folder= r"E:\post gratuation\anji\zBigJumpImgs"
    # pcd_folder = r"E:\post gratuation\anji\imgandpcd\pcds"
    # image_folder = r"E:\post gratuation\anji\imgandpcd\imgs"
