...
def detect_points_nearby(plane_equation, point_cloud_np, exclude_plane=None):
    ...
    if exclude_plane:
        point_cloud_np = point_cloud_np[
            ~np.apply_along_axis(lambda p: is_point_on_plane(exclude_plane, p), axis=1, arr=point_cloud_np)]
...
def visualize_and_filter_point_cloud():
    ...
    """
    # 点云文件路径
    point_cloud_path = point_cloud_file  # 替换为你实际的点云文件路径
    ...
    plane_eq = visualize_with_open3d(point_cloud_np, moved_plane_equations, plane_B_equation, thickness_A,
                                     thickness_B)  # 平面方程：z = 0
    A, B, C, D = plane_eq
...
def create_plane_points(plane_equation, point_cloud_np, thickness, angle_deg):
    ...
    if rotated_normal[2] == 0:
        ...
        # 使用旋转后的法向量来调整Z值的计算
        Z_rotated = (-rotated_normal[0] / rotated_normal[2] * X - rotated_normal[1] / rotated_normal[2] * Y - D /
                     rotated_normal[2])
...
def cut_rail(plane_b_equation, point_tail_np,point_cloud_np):
    print('111')
    # visualize_and_filter_point_cloud()
