...
def call_back(ros_pcd):
    ...
    origin_pcd.points = o3d.utility.Vector3dVector(points[:, 0:3])
    ...
    try:
        ...
        # 获得角点坐标数组以及法向量
        edge_points, row_vector = restore_on_pointCloud(np.asarray(origin_pcd.points), real_coordinates=real_coordinates,
        ...
        o3d.visualization.draw_geometries([ROI_pcd, origin_pcd, axis_pcd], window_name='标定板点云', width=1080,
                                          height=800, left=50, top=50, point_show_normal=False, mesh_show_wireframe=False,
                                          mesh_show_back_face=False)
...
if __name__ == '__main__':
    ...
    print('初始化')
    rospy.Subscriber("/lslidar_point_cloud", PointCloud2, call_back, queue_size=1)
    # 循环来源
